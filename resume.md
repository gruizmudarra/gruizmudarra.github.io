---
layout: page
title: Resumé
permalink: /resume/
---

## Education

**M. Sc. in Mechatronics Engineering**

University of Málaga (Oct. 2020 ‑ Jul. 2021)

- Optional courses focused in:
    * Fuzzy logic and neural networks
    * Mobile robotics
    * Advanced electronic design
    * Movement control in the state‑space
    * Computer vision
    * Telerobotics
- *Thesis: 5G sensor integration using ROS for emergency responce missions.*

**B. Sc. in Electronics, Robotics and Mechatronics Engineering** 

University of Málaga (Oct. 2016 ‑ Sep. 2020)
- Mention: Robotics and automation.
- *Thesis: Autonomous driving in intersections and roundabouts of a 1/10 scale car with ROS and Gazebo.*

## Extracurricular Activity

**Research Engineer**

*UMA Garage Autonomous Driving Team (Feb. 2021 ‑ now)*

- Proposed a system to perform parking and road navigation based on a state‑machine.
- Created a traffic sign detection system using Machine Learning and computer vision.

**Lab assistant** 

*Electronics Department at the University of Málaga (Oct. 2017 ‑ Jan. 2018, 21 hours in total)*

- Supervise lab material
- Give advise to other students

## Skills

**Software**    
:   MATLAB/Simulink, LabVIEW, ROS, OpenCV, Git.

**Electronics** 
:   PCB design (Eagle), Real‑Time microcontroller programming (Atmel Studio), FPGA design (Xilinx Vivado and ISE).

**Programming** 
:   C/C++, Python, JAVA, LaTeX.

**Languages**   
:   English, Spanish.

## Courses & Achievements

2021 
:   **Paper submitted to ICRA 2022 (currently under review)**: *Enabling ROS for Android in a distributedcommunication system for its use in an emergency response robotic application.*

:   **Passed with honors** in the course Movement Control at the M. Sc. in Mechatronics Engineering.

2020 
:   **Certifications in CUDA (single and multi‑GPU) and Deep Learning,** NVIDIA & Deep Learning Intitute &University of Málaga.

2015 
:   **B2 First,** Cambridge English Assessment.